...
from isledecomp.db import ReccmpThing, ReccmpDb
...
class CompareDb(ReccmpDb):
    ...
    def match_vtable(
        ...
        for row in self.search(name=for_vftable, matched=False):
            assert row.target is not None  # TODO: mypy
        ...
        if func.symbol is not None:
            for var_symbol in self.search_symbol(func.symbol):
                ...
                if (
                    obj
                    and not obj.matched
                    and obj.get("type") in (None, SymbolType.DATA)
                ):
                    assert obj.target is not None  # TODO: mypy
